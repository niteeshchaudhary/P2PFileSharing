...
class Peer:
    def __init__(self, manager_ip, manager_port,listen_ip, listen_port, files_directory):
        ...
        self.listener_socket.bind((listen_ip, self.listen_port))
    ...
    def ping_manager(self):
        """
        Send a ping message to the manager to let it know the peer is online.
        """
        message = {'type': 'ping'}
        self.send_message(message, (self.manager_ip, self.manager_port))
    ...
    def send_message(self, message, address):
        ...
        try:
            ...
            self.manager_socket.sendto(message.encode(), address)
    ...
    def register_with_manager(self):
        ...
        message = "{"+"type:"+ "register,"+"address:"+ self.listen_ip+","+"port:"+ self.listen_port+","  + "files:"+ self.get_shareable_files()+","+ "}"
        
        # send message to manager
        self.send_message(message, (self.manager_ip, self.manager_port))
        print(f"Registered with manager at {self.manager_ip}:{self.manager_port}")
...
if __name__ == "__main__":
    ...
    peer = Peer(manager_ip="127.0.0.1", manager_port=5000,listen_ip="127.0.0.1", listen_port=6000, files_directory="shared_files")
